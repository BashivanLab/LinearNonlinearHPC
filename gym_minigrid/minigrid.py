...
COLORS = {
    ...
    # 'steel': np.array([61, 70, 114]),
    'steel': np.array([47, 56, 102]),
}
...
class WorldObj:
    ...
    def decode(type_idx, color_idx, state):
        ...
        if obj_type == 'wall':
            ...
            v = Lava()
        elif obj_type == 'square': #can remove it, only used for debugging
            v = Square(color)
        # elif obj_type == 'triangle': #can remove it, only used for debugging
        #     v = Triangle(color)
        # elif obj_type == 'circle': #can remove it, only used for debugging
        #     v = Circle(color)
        else:
...
class Grid:
    ...
    def render(
        ...
        if highlight_mask is None:
            highlight_mask = np.zeros(shape=(self.width, self.height), dtype=bool) # dtype=np.bool)
    ...
    def decode(array):
        ...
        vis_mask = np.ones(shape=(width, height), dtype=bool) #dtype=np.bool)
    ...
    def process_vis(grid, agent_pos):
        mask = np.zeros(shape=(grid.width, grid.height), dtype=bool)# dtype=np.bool)
    ...
    # 2 functions below are only used by memorygame.py
    @classmethod
    ...
    def render_wo_agent_dir(
        ...
        # # TODO tried add grey highlight for agent vision
        ...
        # for vis_j in range(0, self.agent_info[0]):
        #     for vis_i in range(0, self.agent_info[0]):
...
class MiniGridEnv(gym.Env):
    ...
    def gen_obs_grid(self):
        ...
        if not self.see_through_walls:
            ...
            vis_mask = np.ones(shape=(grid.width, grid.height), dtype=bool) # np.bool)
    ...
    def get_obs_render(self, obs, tile_size=TILE_PIXELS//2):
        ...
        img = grid.render(
            tile_size,
            agent_pos=(self.agent_view_size // 2, self.agent_view_size - 1), # None
            agent_dir=3, # None
