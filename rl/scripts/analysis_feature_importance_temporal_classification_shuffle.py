...
def get_trial_phases(row):
    ...
    if np.isnan(current_row).all():
        # trial_phases of a nan row is nan
...
def get_meta_dataset(neuron_metadata, firing_index, factors_start_index, delete_n_steps_overlap=False,
                     add_trial_phases=False, n_steps=1, shuffle_features_control=False):
    """
    ...
    """
    if shuffle_features_control:
        all_trial_ids = [neuron_metadata[t][sample_index][1] for t in range(len(neuron_metadata))]
    X_metadata = []
    firing_rates = []
    i = 0
    ...
    while i in range(len(neuron_metadata)-future_steps):
        ...
        if row is None:
            ...
            current_features = neuron_metadata[i][factors_start_index:]
            if shuffle_features_control:
                # incomplete_row = False
                trial_id = neuron_metadata[i][sample_index][1]
                within_trial_indices = [index for index, current_id in enumerate(all_trial_ids) if
                                    current_id == trial_id]
                # Remove the correct index
                within_trial_indices.remove(i)
                if len(within_trial_indices) == 0:
                    current_features = list(np.full(len(current_features), np.nan))
                else:
                    #  Get random features within the same trial
                    current_features = neuron_metadata[choice(within_trial_indices)][factors_start_index:]
        ...
        for step in range(1, n_steps):
            ...
            step_i = i - step
            current_features = neuron_metadata[step_i][factors_start_index:]
            if shuffle_features_control:
                trial_id = neuron_metadata[step_i][sample_index][1]
                within_trial_indices = [index for index, current_id in enumerate(all_trial_ids) if
                                    current_id == trial_id]
                # Remove the correct index
                within_trial_indices.remove(step_i)
                if len(within_trial_indices) == 0:
                    current_features = list(np.full(len(current_features), np.nan))
                else:
                    #  Get random features within the same trial
                    current_features = neuron_metadata[choice(within_trial_indices)][factors_start_index:]
            row.extend(current_features)
            if add_trial_phases:
                row.extend(get_trial_phases(current_features))
...
if do_row_wise:
    ...
    if load_previous_row_wise:
        path = "rl/figures/feature_importance/28fmay25_shuffled_temporal_classification_mean_v2_p01_distance10/row_wise_profile_trial_shuffle.pickle"
        number_step = 30
            ...
            for s in range(nb_seeds):
                for neuron_id in metadata.keys():
                    ...
                    X_metadata, firing_rates = get_meta_dataset(neuron_metadata, fr_index, factors_start_index,
                                                                ...
                                                                shuffle_features_control=shuffle_features_control)
                    ...
                    for row_index in range(nb_steps):
                        ...
                        filtered_firing_rates = firing_rates
                        if shuffle_features_control:
                            nan_filter = ~np.isnan(row_i).any(axis=1)
                            row_i = row_i[nan_filter]
                            filtered_firing_rates = firing_rates[nan_filter]
                        predicted_ff, reg_model, kfold_weights = timeseries_crosval_predict(row_i, filtered_firing_rates, reg_model)
                        corr, pvalue = pearsonr(predicted_ff, filtered_firing_rates)
    ...
    if plot_row_wise:
        ...
        for neuron_id in row_wise_corr_per_neuron.keys():
            ...
            # if neuron_id == 'R20141212_Hc9_3':
            #     plt.gca().set_yticklabels(['', '0.00', '0.10', '0.20', '0.30'])
    ...
    for neuron_id in row_wise_corr_per_neuron.keys():
        ...
        current_scores = scores[:, nb_future_steps] # Time steps 0, i.e. the current timesteps
    ...
    if nb_future_steps == 0:
        ...
        perceptual_neurons = np.abs(memory_distance) < distance_threshold
        memory_neurons = memory_distance <= -distance_threshold
        long_future_neurons = memory_distance >= far_distance_threshold
        current_trial_future_neurons = np.logical_and(memory_distance < far_distance_threshold, memory_distance >= distance_threshold)
        temp_dict = {
            'neuron_ids': neuron_names,
            'memory_strength': memory_strength,
            'memory_distance': memory_distance,
            ...
            'current_trial_memory_neurons': current_trial_memory_neurons,
            'long_memory_neurons': long_memory_neurons,
            'long_future_neurons': long_future_neurons,
            'current_trial_future_neurons': current_trial_future_neurons,
        }
